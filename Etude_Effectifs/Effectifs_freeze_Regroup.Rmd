...
#calcul temps adhesion de toutes les personnes dans  une entreprise
  ...
  return (round(as.integer(adh_moyenne)/365,digit=1))
}
#en jour
```
...
tab <- data.frame(Lien_E = integer(nb_liens),
                 ...
                 BASE=numeric(nb_liens), # en numerique
...
for (i in 1:1007){
  ...
  #nb_pers et famille
  x=effectif_pers_fam(sub_lien)
  tab[i,c(15,17,19,21,23)]=x$Nb_pers
  tab[i,c(16,18,20,22,24)]=x$Nb_fam
  ...
  #index prod1
  tab[i,39:44]=index_prod(sub_lien,tab[i,5])
  
  #indexation prod2
  tab[i,45:50]=index_prod(sub_lien,tab[i,6])
  
  #indexation prod3
  tab[i,51:56]=index_prod(sub_lien,tab[i,7])
  
  #renego
  tab[i,57:59]=sub_lien[1,27:29]
}
```
...
```{r asspri}
tab=subset(tab, tab$Asspri_percent !=0.0)
```
